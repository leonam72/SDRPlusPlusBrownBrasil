...
namespace dsp {
    ...
    struct SMAStream {
        ...
        void produceSMA() {
            ...
            if (output.size() < V) { return; }
            complex_t s = { 0.0, 0.0 };
            for (int q = output.size() - V; q < (int)output.size(); q++) {
                s += input[q];
            }
    ...
    struct AFNR_OMLSA_MCRA : public Processor<stereo_t, stereo_t> {
        ...
        void process(stereo_t *readBuf, int count, stereo_t *writeBuf, int &wrote) {
            ...
            if ((int)buffer.size() >= blockSize) {
                ...
                if (scaled < 32757) { scaled += 10; }
                for (int q = 0; q < blockSize; q++) {
                    if (fabs(buffer[q].l) > max) { max = fabs(buffer[q].l); }
                    processIn[q] = buffer[q].l * scaled;
                }
                if (max > 32767.0f / scaled) {
                    float newScaled = 32767.0f / max;
                    for (int q = 0; q < blockSize; q++) { processIn[q] = buffer[q].l * newScaled; }
                    scaled = newScaled;
                }
    ...
    struct AFNRLogMMSE : public Processor<complex_t, complex_t> {
        ...
        // FIX: inicializado como 0 para forcar sincronizacao no primeiro start()
        // Antes era 24000 (48000/2) estatico e nunca era atualizado ao ativar
        int processingBandwidthHz = 0;

        std::mutex freqMutex;

        void setProcessingBandwidth(int bandwidthHz) {
            flog::info("Refreshing noise profile for AF NR (logmmse), bw={}", bandwidthHz);
            freqMutex.lock();
        ...
        void start() override {
            ...
            // FIX: sincroniza processingBandwidthHz com o VFO real ao ativar
            // Antes permanecia 24000 estatico causando perfil de ruido errado
            // na primeira ativacao ou apos trocar de VFO
            double vfoBw = getVFOBandwidth();
            if (vfoBw > 0 && (int)vfoBw != processingBandwidthHz) {
                flog::info("AFNRLogMMSE::start() syncing bw from VFO: {} Hz", (int)vfoBw);
                setProcessingBandwidth((int)vfoBw);
            } else if (processingBandwidthHz <= 0) {
                // fallback seguro se VFO ainda nao esta disponivel
                setProcessingBandwidth(48000 / 2);
            }
            lastVFOBandwidth = vfoBw;
        ...
        void process(complex_t *readBuf, int count, complex_t *writeBuf, int &wrote) {
            ...
            if (!params.noise_mu2) {
                if ((int)worker1c->size() >= noiseFrames * Slen) {
                    ...
                    worker1c->erase(worker1c->begin(), worker1c->begin() + curSize);
                } else {
            ...
            if ((int)worker1c->size() >= 4 * params.Slen && params.noise_mu2) {
        ...
        int run() override {
            ...
            if (getVFOBandwidth() != lastVFOBandwidth) {
                ...
                // FIX: ao mudar BW do VFO em tempo real, atualiza processingBandwidthHz
                // tambem (antes so resetava o perfil mas mantinha bw errada)
                setProcessingBandwidth((int)lastVFOBandwidth);
            }
